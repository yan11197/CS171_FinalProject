...
StatsOverTime.prototype.updateVis = function(year1, year2) {
    ...
    vis.xAxis.tickFormat(function(d) {return d});
    ...
    function insert_line(Line_class, Circle_class, worst_average_best, t) {
        ...
        // console.log(vis.filteredData.slice(year1-1996, year2-1996+1))
    ...
    // Add the tool tips
    var tool_tip_best = d3.tip()
        .attr("class", "d3-tip")
        .offset([-8, 0])
        .html(function(d) {
            // To split the name to only get the full name
            var qb = d[Stat]['best'][1];
            ...
            // Return
            return '<b>' + qb_name + '</b>: <br/>Attempts: ' + att + '<br/>Completions: ' + cmp +
                '<br/>Touchdowns: ' + td + '<br/>Interceptions: ' + int
        });

    // Creating and calling the tool tip
    var tool_tip_worst = d3.tip()
        .attr("class", "d3-tip")
        .offset([-8, 0])
        .html(function(d) {
            // To split the name to only get the full name
            var qb = d[Stat]['worst'][1];
            var qb_name = qb.slice(0, qb.lastIndexOf(".")-1);
    ...
    vis.svg.call(tool_tip_best);
    vis.svg.call(tool_tip_worst);

    vis.svg.selectAll(".average_circle")
        .on('mouseover', function(d) {})

    vis.svg.selectAll(".best_circle")
        .on('mouseover', tool_tip_best.show)
        .on('mouseout', tool_tip_best.hide)
        .on('click', function(d) {
            vis.makeLegend(0)
            vis.updateClick(d, Stat, true, year1, year2)});

    vis.svg.selectAll(".worst_circle")
        .on('mouseover', tool_tip_worst.show)
        .on('mouseout', tool_tip_worst.hide)
        .on('click', function(d) {
            vis.makeLegend(0)
